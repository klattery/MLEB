...
env_code$make_con <- function(code_master = code_master, col_pos = NULL, col_neg = NULL, row_rel = NULL){
...
env_code$check_beta <- function(beta, constrain){
  ...
  if (sum(!result$Inside) == 0){
    message("Test value is within constraints")
    message("No changes needed in Step 4")
    consistent <- TRUE
  } else {
    if (sum(!result$NoViol) == 0){
      message("Test value does not violate constraints")
      message("But it is on Constraint Boundary (will not work for intial value)")
      message("Must set initial value x0 in step 4")
    } else {
      message("Test value violates constraints")
      message("Must set initial value x0 in step 4")
    } 
  } 
  result2 <- list(result = result, consistent = consistent)
  return(result2)
}
...
env_eb$agg_solve <- function(data_list, model_list, fish_inf = FALSE) {
  ...
  #ksolve$agg_stats <- as.data.frame(colnames(data_list$ind))
  ksolve$betas <- as.matrix(round(ksolve$par,5))
  #colnames(ksolve$agg_stats) = c("vnames", "betas")
...
env_eb$mleb <- function(data_list, model_list, mleb_control){
  ...
  if (mleb_control$solveonly) {
    ...
    internal_hold <- jack_knife(data_list$idtask, mleb_control$hold_tasks, mleb_control$hold_resp) # select calibration tasks per respondent
    ...
    for (iter in 1:mleb_control$maxiter) {
      ...
      cal_filter <- jack_knife(idtask = data_list$idtask, num_round = mleb_control$cal_tasks, num_resp = mleb_control$cal_resp) # select calibration tasks per respondent
      ...
      #Solution of Betas
      eb_betas <- t(sapply(1:length(data_list$resp_id), function(x)
        SolveID(idseq = x, data_list=data_list, model_env = model_env)$betas))
      ...
      dev.copy(device=pdf,paste0(mleb_control$dir_pdf,"MLEB_Plot_iter_", iter, ".pdf"), width = 10, height = 6)
      dev.off()
...
env_eb$next_cov <- function(prior_alpha, betas,
  ...
  ab_diff <- t(apply(betas, 1, function(x) x - xbar))
...
env_eb$ll_out <- function(kscale, cal_filter, resp_est, 
  ...
  for (i in 1:ncol(cal_filter)){
    pred_s <- do.call(rbind, lapply(resp_est,
                                    function(x) SolveID(idseq = x, data_list=data_list, model_env = model_env, sample_est = cal_filter[, i])$predprob))
...
env_eb$jack_knife <- function(idtask, num_round = 1, num_resp = NULL) {
  # num_resp = NULL gives all respondents
  idtask_u <- unique(idtask)
  ksplit <- split(idtask_u, idtask_u[, 1])
  if (is.null(num_resp)) {
...
###############  General Prep ###############
env_eb$prep_file <- function(idtaskdep, ind) {
...
env_eb$select_train <- function(idtask, num_hold = 1, num_resp = NULL) {
  # num_resp = NULL gives all respondents
  idtask_u <- unique(idtask)
  ksplit <- split(idtask_u, idtask_u[, 1])
  if (is.null(num_resp)) {
  ...
  pick_hold <- do.call(rbind, lapply(ksplit[resp_pick], function(x) x[sample(1:nrow(x), num_hold),]))
  kmatch <- match(data.frame(t(idtask)), data.frame(t(pick_hold))) # match holdout tasks
  trainhold <- is.na(kmatch) # boolean vector specifying whether train or holdout
  return(trainhold)
}

# Custom function for Unspoken.  Test weights in seq_test for each respondent
env_eb$SolveID_TestWts <- function(idseq, data_list, model_env, seq_test) {
  ...
  cal_n <- min(floor(length(task_id_u) * 1), 10) # 10 Validations is plenty 
  cal_tasks <- sample(id_list$idtask_r, cal_n)
  sample_est <- as.matrix(!sapply(cal_tasks, function(x)(id_list$idtask_r == x)))
  # sample_est must be matrix with >= 1 column
  wts_use <- data_list$wts
  id_list$wts <- wts_use[id_filter]
